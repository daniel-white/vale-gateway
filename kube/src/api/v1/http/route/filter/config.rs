...
use vg_http_config::filter::access_control::AccessControlFilterRef as AccessControlFilterRefConfig;
...
use vg_http_config::filter::{
    AccessControlRouteFilter, ErrorResponseRouteFilter, RedirectResponseRouteFilter,
    RequestHeaderModifierRouteFilter, RouteFilter, StaticResponseRouteFilter,
...
impl TryFrom<HTTPRouteFilterWrapper<'_>> for RouteFilter {
        ...
        match (
            ...
            (HTTPFilterType::ExtensionRef, Some(extension_ref), None, None, None, None) => {
                let extension_ref = ExtensionRefWrapper::builder()
                    ...
                    .build();
                RouteFilter::try_from(extension_ref).map_err(RouteFilterConversionError::Extension)
            }
            (
...
impl TryFrom<ExtensionRefWrapper<'_>> for RouteFilter {
    ...
    fn try_from(value: ExtensionRefWrapper) -> Result<Self, Self::Error> {
        ...
        if let Ok(filter_ref) = filter_ref {
            ...
            let filter = AccessControlRouteFilter::builder().ref_(filter_ref).build();
        ...
        if let Ok(filter_ref) = filter_ref {
            ...
            let filter = ErrorResponseRouteFilter::builder().ref_(filter_ref).build();
            return Ok(filter.into());
        }
        ...
        if let Ok(filter_ref) = filter_ref {
            ...
            let filter = StaticResponseRouteFilter::builder()
                .ref_(filter_ref)
                .build();
            return Ok(filter.into());
        }

...
use vg_rpc_client::{ConfigurationClient, ConfigurationEvent, ConfigurationEventReceiver, ConfigurationEventRecvError};
...
impl ConfigurationRegistryEventReceiver {
    pub async fn recv(&mut self) -> Result<ConfigurationRegistryEvent, ConfigurationRegistryRecvEventError> {
...
impl ConfigurationRegistry {
    ...
    async fn send_http_configuration_changed(&mut self) {
        ...
        if let Some(listener) = listener {
            let routes: HashMap<_, _> = self.routes.iter().map(|e| (e.key().clone(), e.value().clone())).collect();
            let snapshot = HttpConfigurationSnapshot::builder().listener(listener).routes(Arc::new(routes)).build();
    ...
    async fn send_backend_configuration_changed(&mut self) {
        let backends: HashMap<_, _> = self.backends.iter().map(|e| (e.key().clone(), e.value().clone())).collect();
        let snapshot = BackendConfigurationSnapshot::builder().backends(Arc::new(backends)).build();
    ...
    async fn set_backend(&mut self, backend_ref: BackendRef, backend: Backend) {
        let _ = self.backends.insert(Arc::new(backend_ref), Arc::new(backend));
...
impl ConfigurationRegistrySynchronizer {
    ...
    async fn seed(client: &ConfigurationClient, registry: &mut ConfigurationRegistry) {
        match client.listener().await {
            Ok(listener) => registry.set_listener(listener).await,
            Err(_) => {}
        }
    }
}
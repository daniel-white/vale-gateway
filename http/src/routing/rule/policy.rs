use crate::policy::{
    RetryPolicy, RetryPolicyConversionError, TimeoutPolicy, TimeoutPolicyConversionError,
};
use getset::{CloneGetters, Getters};
use thiserror::Error;
use typed_builder::TypedBuilder;
use vg_http_config::routing::rule::policy::{
    RulePolicies as RulePoliciesConfig, TimeoutPolicies as TimeoutPoliciesConfig,
};
...
pub enum RulePoliciesConversionError {
    ...
    RetryPolicy(#[from] RetryPolicyConversionError),
}
...
pub enum TimeoutPoliciesConversionError {
    ...
    RequestPolicy(TimeoutPolicyConversionError),

    #[error("backend request timeout policy is invalid: {0}")]
    BackendRequestPolicy(TimeoutPolicyConversionError),
}
